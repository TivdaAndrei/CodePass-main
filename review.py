...
def main():
    ...
    if args.directory:
        console.print(f"[bold cyan]📁 Analyzing directory: {args.directory}[/bold cyan]")
        
        python_files = glob.glob(os.path.join(args.directory, '**/*.py'), recursive=True)

        if not python_files:
            console.print("[yellow]⚠️  No .py files found in the specified directory.[/yellow]")
            return

        console.print(f"[cyan]Found {len(python_files)} Python file(s) to review[/cyan]\n")

        for filepath in python_files:
            console.print(f"\n[bold yellow]🔍 Analyzing: {filepath}[/bold yellow]")
            ...
            if not code_to_review.strip():
                console.print("[italic]⊘ Empty file, skipping...[/italic]")
                continue

            # Live reviewing with real-time streaming
            console.print("[cyan]⏳ Streaming live code review...[/cyan]")
    ...
    # ELSE LOGIC (Existing functionality)
        ...
        if args.files:
            console.print(f"[bold cyan]📄 Analyzing {len(args.files)} file(s)...[/bold cyan]\n")
            
            for file_path in args.files:
                try:
                    with open(file_path, 'r', encoding='utf-8') as f:
                        filepath_str = file_path
                        console.print(f"\n[bold yellow]🔍 Analyzing: {filepath_str}[/bold yellow]")
                        code_to_review = f.read()

                        if not code_to_review.strip():
                            console.print("[italic]⊘ Empty file or input, skipping...[/italic]")
                            continue
        ...
        if not sys.stdin.isatty():
            ...
            console.print("\n[bold yellow]🔍 Analyzing standard input (stdin)[/bold yellow]")
            console.print("[cyan]⏳ Streaming live code review...[/cyan]\n")
